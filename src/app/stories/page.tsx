...
import { FaCamera, FaHeart, FaComment, FaShare, FaPlus, FaTimes, FaSpinner } from 'react-icons/fa';
import MainNav from '@/components/layout/MainNav';
import ReputationScore from '@/components/ui/ReputationScore';
import { useStories, useCreateStory, useViewStory } from '@/hooks/api';
    ...
    >
      <div className="flex items-center justify-between p-4">
        ...
        <button 
          ...
          onClick={() => {
            if (!mediaUrl && captureMode !== '3d') {
              toast.error('Please capture or select media first');
              return;
            }
            
            // In a real implementation, you would upload the media file first
            // and then pass the URL to the createStory function
            // For demo purposes, we'll use placeholder URLs based on the type
            const demoUrls = {
              'image': 'https://images.unsplash.com/photo-1620641788421-7a1c342ea42e',
              'video': 'https://assets.mixkit.co/videos/preview/mixkit-digital-animation-of-a-city-at-night-11748-large.mp4',
              '3d': '/3d-models/sample.glb'
            };
            
            createStory({
              mediaUrl: mediaUrl || demoUrls[captureMode],
              mediaType: captureMode === '3d' ? 'image' : captureMode, // API only supports image and video for now
              caption: caption || undefined,
              duration: captureMode === 'video' ? 8 : 5 // Default durations
            });
...
export default function StoriesPage() {
  ...
  // Use static stories as fallback when API data is loading or empty
  const [staticStories, setStaticStories] = useState<Story[]>([
    {
      id: 'story1',
      user: {
        id: 'user1',
  ...
  // Combine API stories with static stories if needed
  const displayStories = storiesLoading || stories.length === 0 ? staticStories : stories;
  
  // Mark a story as viewed when opened
  const handleOpenStory = (index: number) => {
    setActiveStoryIndex(index);
    
    // If we're using real API data, call the viewStory endpoint
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {storiesLoading && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="rounded-lg bg-gray-900 p-6 shadow-xl">
            ...
            <p className="mt-4 text-center text-white">Loading stories...</p>
          </div>
        </div>
      )}
      ...
      {/* Error message */}
      {storiesError && (
        <div className="mt-4 rounded-lg bg-red-500/10 p-4 text-center text-red-500">
